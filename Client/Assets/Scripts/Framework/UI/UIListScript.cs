...
public class UIListScript : UIComponent
{
    ...
    public GameObject ZeroTipsObj;

    protected int m_selectedElementIndex = -1;

    protected int m_lastSelectedElementIndex = -1;

    public bool alwaysDispatchSelectedChangeEvent;

    [HideInInspector]
    public enUIEventID listSelectChangedEventID;

    public stUIEventParams listSelectChangedEventParams;

    [HideInInspector]
    public enUIEventID listScrollChangedEventID;

    public stUIEventParams listScrollChangedEventParams;
    ...
    [HideInInspector]
    public bool autoAdjustScrollAreaSize;
    ...
    public virtual void SetElementAmount(int amount, List<Vector2> elementsSize)
    {
        ...
        if (ZeroTipsObj != null)
        {
            if (amount <= 0)
            {
                ZeroTipsObj.SetActive(true);
            }
            else
            {
                ZeroTipsObj.SetActive(false);
            }
        }
    }
    ...
    {
        if (extraContent != null && m_isExtraContentEnabled)
        {
    ...
    {
        if (listSelectChangedEventID != enUIEventID.None)
        {
            UIEvent uIEvent = Singleton<UIEventManager>.GetInstance().GetUIEvent();
            uIEvent.eventID = listSelectChangedEventID;
            uIEvent.eventParams = listSelectChangedEventParams;
    ...
    {
        if (listScrollChangedEventID != enUIEventID.None)
        {
            UIEvent uIEvent = Singleton<UIEventManager>.GetInstance().GetUIEvent();
            uIEvent.eventID = listScrollChangedEventID;
            uIEvent.eventParams = listScrollChangedEventParams;
            uIEvent.srcFormScript = belongedFormScript;
            uIEvent.srcWidgetBelongedListScript = belongedListScript;
            uIEvent.SrcWidgetIndexInBelongedList = indexInList;
    ...
    {
        RectTransform rectTransform = extraContent.transform as RectTransform;
        rectTransform.anchoredPosition = offset;
        extraContent.SetActive(true);
        if (listType == enUIListType.Horizontal || listType == enUIListType.HorizontalGrid)
        {
            contentSize.x += rectTransform.rect.width + elementSpacing.x;
        }
        else
        {
            contentSize.y += rectTransform.rect.height + elementSpacing.y;
        }
    }
