...
public class TableResMgr : Singleton<TableResMgr>
{
    ...
    public Dictionary<int, CTableRecordBase> mTablePathDic = new Dictionary<int, CTableRecordBase>()
    {
        {
            IdxHint,
            new TableRecord<chapter_info,chapter_info_ARRAY>
            {
                mTableIndex = IdxHint,
                mTablePath = "Databin/TableRes/chapter_info",
            }
         },
    };
...
public class CRecordTable
{
    ...
    public void Load<TItem, TList>(byte[] buffer)
    {
        try
        {
            ...
            {
                TList list = Serializer.Deserialize<TList>(stream);
                if (list == null)
                {
                ...
                PropertyInfo listProperty = list.GetType().GetProperty("items");
