...
class SheetInterpreter:
    ...
    def Interpreter(self) :
        ...
        self._output.append("package TableProto;\n")
    ...
    def _LayoutStructHead(self, struct_name) :
        ...
        pblist.append("\n")
        pblist.append(" "*self._indentation + "message " + struct_name + "{\n")
    ...
    def _LayoutStructTail(self) :
        ...
        self._output.append("\n")
        pblist.append(" "*self._indentation + "}\n")
        pblist.append("\n")
    ...
    def _LayoutComment(self, comment) :
        ...
        if comment.count("\n") > 1 :
            if comment[-1] != '\n':
                ...
                pblist.append(" "*self._indentation + "/** " + comment + " "*self._indentation + "*/\n")
        else :
            self._output.append(" "*self._indentation + "/** " + comment + " */\n")
            pblist.append(" "*self._indentation + "/** " + comment + " */\n")
    ...
    def _LayoutOneField(self, field_rule, field_type, field_name) :
        ...
        if field_name.find('=') > 0 :
            ...
            file_index = self._GetAndAddFieldIndex()
            self._output.append(" "*self._indentation + field_rule + " " + field_type \
                    + " " + str(name_and_value[0]).strip() + " = " + file_index\
                    + " [default = " + str(name_and_value[1]).strip() + "]" + ";\n")
            pblist.append(" "*self._indentation + field_rule + " " + field_type \
                    + " " + str(name_and_value[0]).strip() + " = " + file_index\
                    + " [default = " + str(name_and_value[1]).strip() + "]" + ";\n")
            return
    ...
    def _LayoutArray(self) :
        ...
        pblist.append("message " + self._sheet_name + "_ARRAY {\n")
        pblist.append("    repeated " + self._sheet_name + " items = 1;\n}\n")
...
class DataParser:
    ...
    def _ParseField(self, max_repeated_num, repeated_num, item) :
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            #测试
            #print ("parse cell(%u, %u) type: %s"%(self._row, self._col, str(self._sheet.cell_value(self._row, self._col))))
    ...
    def _GetFieldValue(self, field_type, row, col) :
        ...
        try:
            if field_type == "int32" or field_type == "int64"\
                ...
                #整数数字文本 去掉变为float的小数部分
                if self._sheet.cell_type(row, col) == 2:
                    field_value = field_value.replace(".0", "")

                #print ("parse cell(%u, %u) num: %s, %s"%(row, col, field_value, self._sheet.cell_type(row, col)))
...
if __name__ == '__main__' :
    xls_file_path = "../../../../ConfigFile/Excel/"
    fileList = glob.glob(xls_file_path + "/*.xls")
    try :
        for file in fileList:
            print file
            filename = file
            filename = filename.replace("../../../../ConfigFile/Excel\\","")
